...
try:
    df_THYAO = pd.read_sql("SELECT * FROM thyao_df", connection)
    df_interest_rates = pd.read_sql("SELECT * FROM interest_df", connection)
except Exception:
    print("Could not find a database, please run detch_data.py")
    exit()

df_THYAO["Date"] = pd.to_datetime(df_THYAO["Date"])

df_interest_rates["Tarih"] = pd.to_datetime(df_interest_rates["Tarih"])
...
df_interest_rate_change_pos_dates = df_interest_rate_change_pos["Tarih"]
df_interest_rate_change_neg_dates = df_interest_rate_change_neg["Tarih"]
...
ax1.plot(df_THYAO["Date"], df_THYAO['THYAO.IS'], color='black', label='THYAO')
...
# RSI
ax2.plot(df_THYAO["Date"], df_THYAO['RSI'], color='purple', label='RSI (14)')
...
plt.tight_layout()
plt.savefig(os.path.join(folder_name, "thyao.png"),
